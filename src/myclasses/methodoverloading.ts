...
class Calc{
    ...
    //overload methods: signatures/prototypes: no method body:
    add(a: number, b: number): number;
    add(a: string, b: string): string;
    add(a: string, b: boolean): string;
    add(a: boolean, b: string): boolean;
...
class Ecomm{
    ...
    search(name: string, colorOrPrice: string | number, seller?: string, zip?: number): string[] {
        ...
        console.log(`${name} - ${colorOrPrice} - ${zip}`);
...
ec.search('Apple', 'black', 'flipkart', 90900);
...
function login(username: string, otpOrPassword: string | number, role?: string) {
...
console.log(login('naveen', 'naveen123', 'admin'));
